class Admin::CommentsController < ApplicationController
  ...
  def index
    if username = params[:username]
      ...
      @comments = @user.comments.desc(:created_at).page(params[:page] || 1).per(20)
    elsif entry_id = params[:entry_id]
      @entry = Entry.where(:id => entry_id).first
      @comments = @entry.comments.desc(:created_at).page(params[:page] || 1).per(20)
    else
      @comments = Comment.desc(:created_at).page(params[:page] || 1).per(20)
    end
